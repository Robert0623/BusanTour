...
<%--<form action="<c:url value='/register/add'/>" method="post" onsubmit="return formCheck(this)">--%>
  ...
  <label for="">아이디</label>
  <input class="input-field" type="text" name="id" placeholder="4~12자리의 영대소문자와 숫자 조합" autofocus>
...
<script>
  function formCheck(frm) {
    ...
    if(frm.id.value.length<4) {
      setMessage('id의 길이는 4 ~ 12이어야 합니다.', frm.id);
      return false;
    }
    if(frm.pwd.value.length<4) {
      setMessage('pwd의 길이는 4 ~ 12이어야 합니다.', frm.pwd);
      return false;
    }
